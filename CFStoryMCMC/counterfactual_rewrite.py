...
from ..tools import load_json_samples
...
class EduCat():
    ...
    def _load_data(self, samples):
        ...
        self.dataset.load(samples)

    def sample(self, samples, output_file):
        ...
        for sen_id in range(len(self.dataset)):
            ...
            try:
                ...
                for i, ending in enumerate(self.dataset.data['split_original_end'][sen_id]):
                ...
                # generated_endings = 'This is a test sentence.'
                prob_info = ' '.join(map(str, probs_list))
                self.logger.warning(f'{sen_id} [final] {generated_endings}')
            except Exception as e:
                logging.error(e)
                generated_endings = '<|Exception|>'
                prob_info = 'None'
            d = {
                'xid': sen_id,
                ...
                'original_ending': self.dataset.data['original_ending'][sen_id],
                'edited_endings': self.dataset.data['edited_endings'][sen_id],
                'predicted': generated_endings,
                'prob_info': prob_info,
    ...
    def sample_educat(self, data_i, premise, initial, counterfactual, ending, prev_orig_endings, prev_edited_endings):
        ...
        old_prob *= self.stationary.coherence_constraint(
            premise['text'],
            initial['text'] + prev_orig_endings['text'],
            counterfactual['text'] + prev_edited_endings['text'],
            ending['text'], score_method=self.score_method)
        ...
        for iter in range(self.sample_time):
            ...
            if self.causal_token_finding == 'causal':
            ...
            new_prob *= self.stationary.coherence_constraint(
                premise['text'],
                initial['text'] + prev_orig_endings['text'],
                counterfactual['text'] + prev_edited_endings['text'],
                ending_temp, score_method=self.score_method)
...
if __name__ == '__main__':
    ...
    num_partitions = args.num_gpus * num_workers
    datasize = len(samples)
    part_id = args.gpu_id * num_workers + args.worker_id
    chunk_start, chunk_end = (datasize * part_id) // num_partitions, (datasize * (part_id + 1)) // num_partitions
    print('part {}, start {}, end {}'.format(args.gpu_id, chunk_start, chunk_end))
    device = torch.device('cuda:{}'.format(args.gpu_id))
    chunk_samples = samples[chunk_start:chunk_end]

    # np.random.seed(cfg.seed)
    try:
        bert_scorer = BERT_Scorer(args.mlm_path, device)
        stationary = StaDist(args.gpt2_path, args.coherence_type, args.constraint_model_path, device)
        educat = EduCat(args, bert_scorer=bert_scorer, stationary=stationary)
        # outfile = os.path.join(cfg.output_dir, cfg.output_file)
        educat.sample(chunk_samples, args.output_file)
        # if args.pool_size == 1:
        #     results = evaluate(args.output_file, args.data_path, ['entailscore', 'bleu', 'bertscore'])
        #     print(results)
    except Exception as e:
        exc = traceback.format_exc()
        msg = f'[Error]: {e}.\n[Traceback]: {exc}'
        print(msg)
        if args.lark and args.rank_0: cjj.lark(msg)

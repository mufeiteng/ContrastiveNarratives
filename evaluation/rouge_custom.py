...
def _corpus_level_rouge_1(list_of_hypos, list_of_refs, alpha=0.0):
    res = []
    for i in range(len(list_of_hypos)):
        s = rouge_1(peer=list_of_hypos[i], models=list_of_refs[i], alpha=alpha)
        res.append(s)
    return sum(res)/len(res)


def _corpus_level_rouge_2(list_of_hypos, list_of_refs, alpha=0.0):
    res = []
    for i in range(len(list_of_hypos)):
        s = rouge_2(peer=list_of_hypos[i], models=list_of_refs[i], alpha=alpha)
        res.append(s)
    return sum(res)/len(res)


def _corpus_level_rouge_l(list_of_hypos, list_of_refs, alpha=0.0):
    res = []
    for i in range(len(list_of_hypos)):
        s = rouge_l(peer=list_of_hypos[i], models=list_of_refs[i], alpha=alpha)
        res.append(s)
    return sum(res)/len(res)


def corpus_rouge_moses(list_of_hypos, list_of_refs, alpha=0.5):
    rouge1 = _corpus_level_rouge_1(
        list_of_hypos=list_of_hypos, list_of_refs=list_of_refs, alpha=alpha)
    rouge2 = _corpus_level_rouge_2(
        list_of_hypos=list_of_hypos, list_of_refs=list_of_refs, alpha=alpha)
    rougel = _corpus_level_rouge_l(
        list_of_hypos=list_of_hypos, list_of_refs=list_of_refs, alpha=alpha)
